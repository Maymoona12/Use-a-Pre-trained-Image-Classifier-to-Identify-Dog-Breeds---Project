...
def adjust_results4_isadog(results_dic, dogfile):
    ...
    Parameters:
     ...
     dogfile - A text file that contains names of all dogs from the classifier
               ...
               maltese) (string - indicates the text file's filename)
    Returns:
    ...
    # Read in dog names from dogfile, removing newline characters
    with open(dogfile, "r") as infile:
        for line in infile:
            dogname = line.strip()
            if dogname not in dognames_set:
                dognames_set.add(dogname)

    # Update results_dic to indicate whether labels are of-a-dog or not
    for key in results_dic:
        # Determine if pet image label is a dog
        is_pet_dog = 1 if results_dic[key][0] in dognames_set else 0
        # Determine if classifier label is a dog
        is_classifier_dog = 1 if results_dic[key][1] in dognames_set else 0
        # Append results to each entry in results_dic
